...
// ─── Vista de cola local (pendiente / listo) ──────────────────────────────────
...
// ─── Tarjeta del mecánico (solo 3 campos de Supabase) ─────────────────────────
function MotoMecanicoCard({ moto }: { moto: MotoMecanico }) {
...
// ─── Vista de ingresos desde Supabase (mecánico) ─────────────────────────────
function SupabaseIngresosList() {
  ...
  const horaRefresh = lastRefresh.toLocaleTimeString('es-CO', {
    hour: '2-digit',
    minute: '2-digit',
    second: '2-digit',
  });
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-2xl text-center px-6">
        ...
        <button
          onClick={fetchMotos}
          className="flex items-center gap-2 px-5 py-2 bg-auteco-red text-white text-sm font-bold rounded-xl hover:bg-red-700 transition-colors"
        >
      ...
      {motos.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-600 mt-1">
            Usa el módulo de Recepción para ingresar motos.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {motos.map(moto => (
            <MotoMecanicoCard key={moto.id} moto={moto} />
          ))}
        </div>
      )}
    </div>
...
export default function MechanicDashboard() {
  const [mainTab, setMainTab] = useState<'supabase' | 'local'>('supabase');
